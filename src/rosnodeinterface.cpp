...
// C/C++ source code generated on : Thu Oct 26 09:54:51 2023
//
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"carsimplesimulink\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(carsimplesimulink_M, (NULL));
        carsimplesimulink_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(carsimplesimulink_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(carsimplesimulink_M)
                == (NULL)) || rtmGetStopRequested(carsimplesimulink_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(carsimplesimulink_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
